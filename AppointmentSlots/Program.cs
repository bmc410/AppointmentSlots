...
namespace AppointmentSlots
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            int duration = appointmentTypes.Where(x => x.ApptTypeId == 2).FirstOrDefault().ApptLength;
            slots = GetSlots(duration);
            appts = GetAppointments(today,3);
        ...
        static List<ApptModel> GetAppointments(DateTime apptDate, int NumMonths)
        {
            using (SlotsTableEntities db = new SlotsTableEntities())
            {
                List<ApptModel> appts = new List<ApptModel>();
                appts = (from a in db.usp_GetAppointmentsByMonth(apptDate,NumMonths)
